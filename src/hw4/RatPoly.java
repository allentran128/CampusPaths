...
public final class RatPoly {
  ...
  public RatPoly(RatTerm rt) {
    /* not in spec
    if (rt.getExpt() < 0) {
      throw new IllegalArgumentException("Improper rt, with neg exp");
    }
    */
    
    // Check for zero case
    this.terms = new ArrayList<RatTerm>();
    if (!rt.equals(RatTerm.ZERO)) {
      // Add the term in
      terms.add(rt);
    }
    checkRep();
  ...
  private RatPoly(List<RatTerm> rt) {
    ...
    // Check for zero case
    if (this.terms.contains(RatTerm.ZERO)) {
      this.terms.remove(RatTerm.ZERO);
    }
  ...
  public int degree() {
    ...
    // Knowing that size > 0 and descending order of expt
  ...
  public RatTerm getTerm(int deg) {
    /* not in spec
    if (this.isNaN()) {
      throw new IllegalStateException("RatPoly contains NaN term");
    }
    */
  ...
  private static void scaleCoeff(List<RatTerm> lst, RatNum scalar) {
    ...
    */
    ...
    for (RatTerm t : lst) {
      temp.add(new RatTerm(t.getCoeff().mul(scalar), t.getExpt()));
    }
    lst.clear();
    lst.addAll(temp);
  }
  ...
  private static void incremExpt(List<RatTerm> lst, int degree) {
    ...
    */
    ...
    List<RatTerm> temp = new ArrayList<RatTerm>();
    for (RatTerm t: lst) {
      temp.add(new RatTerm(t.getCoeff(), t.getExpt() + degree));
    }
    lst.clear();
    lst.addAll(temp);
  }
  ...
  private static void sortedInsert(List<RatTerm> lst, RatTerm newTerm) {
    ...
    */
     ...
     * newTerm is not NaN
     *          is NaN
     * check first item in lst
     *      if the first item is already less than
     *      the expt of new term, prepend new term to front
     * else general case
     *      go look at every element in the lst
     *      and see if the degree equals 
     *              then combine terms
     *      ELSE 
     *      check if the degree > the current element
     *            then insert in that spot idx == idx(curr)
     * else append at the end
     *      degree is < all elements in the lst, append at 
     *      the end of the list
     */
    
    int degree = newTerm.getExpt();
    
    // adding NaN element case
    if (degree == 0 && newTerm.isNaN()) {
      lst.clear();
      lst.add(RatTerm.NaN);
    } else {
      // empty lst case
      if (lst.size() == 0) {
        ...
        if (degree > lst.get(0).getExpt()) {
          // insert newTerm in the front
          lst.add(0, newTerm);
        } else if (degree < lst.get(lst.size() - 1).getExpt()) {
          // check last element, if degree is smaller than last element
          // append to the end
          lst.add(newTerm);
        } else {
          // general case
          for(int i = 0; i < lst.size(); i++) {
            int currDegree = lst.get(i).getExpt();
            
            if(currDegree == degree) {
              // combine terms
              lst.add(i, newTerm.add(lst.get(i)));
              
              // remove the old term that was pushed back one idx
              lst.remove(i + 1);
              break;
            } else if (degree > currDegree) {
  ...
  public RatPoly negate() {
    ...
    for (RatTerm t : this.terms) {
      temp.terms.add(RatTerm.ZERO.sub(t));
    }
    return temp;
  }

  private void cleanRI (List<RatTerm> lst) {
    // need to check lst against RI:
    // forall i such that (0 <= i < length(p) - 1), E(p,i) > E(p, i+1)
     lst.sort(new Comparator<RatTerm>() {
      @Override
      public int compare(RatTerm r1, RatTerm r2) {
        // descending order
        return Integer.compare(r2.getExpt(), r1.getExpt());
      }
    });
    
    //check for negative exps and zero coeffs
    for (int i = 0; i < lst.size(); i++) {
      if (lst.get(i).getExpt() < 0 || 
          lst.get(i).getCoeff().equals(RatNum.ZERO)) {
        lst.remove(i);
        i--;
      }
    }
  ...
  public RatPoly add(RatPoly p) {
    ...
    */
    ...
    RatPoly temp = this.copy();
    
    for (RatTerm t : p.terms) {
      sortedInsert(temp.terms, t);
      cleanRI(temp.terms);
    }
    return temp;
  }
  ...
  public RatPoly mul(RatPoly p) {
    ...
    */
    ...
    // General case
    List<RatPoly> partialSums = new ArrayList<RatPoly>();
    for (RatTerm t : p.terms) {
      RatNum scalar = t.getCoeff();
      int degree = t.getExpt();
      
      RatPoly temp = this.copy();
      
      // take the p term and multiply with this.terms
      scaleCoeff(temp.terms, scalar);
      incremExpt(temp.terms, degree);
      partialSums.add(temp);
    }
    
    RatPoly product = new RatPoly();
    for (RatPoly r : partialSums) {
      // add all the partials
      product = product.add(r);
    }
  ...
  public RatPoly div(RatPoly p) {
    ...
    */
    ...
    RatPoly quo = new RatPoly();
    RatPoly dividend = new RatPoly(this.terms);
    
    // Used for div alg
    RatTerm pFirst = p.terms.get(0); // constant RatTerm
    int startingExp = dividend.terms.get(0).getExpt(); // starting degree
    
    while (pFirst.getExpt() <= startingExp) {
      // Check if dividend contains a term with the startingExp
      RatTerm t = dividend.getTerm(startingExp);
      if (t != RatTerm.ZERO) {
        // successfully found a term
        RatTerm divFactor = t.div(pFirst);
        RatPoly divTerm = new RatPoly(divFactor);
        
        // Update the remaining dividend
        RatPoly subFactor = p.mul(divTerm);
        dividend = dividend.sub(subFactor);
        
        // Save result into quo
        quo.terms.add(divFactor);
      } else {
        // no term, so check next degree
      }
      startingExp--;
    }
  ...
  public RatPoly differentiate() {
    ...
    // General case
    for (RatTerm t : this.terms) {
      RatTerm grad = t.differentiate();
      if (grad.equals(RatTerm.ZERO)) {
        // dont add anything, want RatPoly.ZERO
      } else {
        temp.terms.add(grad);
      }
    }
  ...
  public RatPoly antiDifferentiate(RatNum integrationConstant) {
    ...
    */
    ...
    RatPoly temp = new RatPoly();
    for (RatTerm t : this.terms) {
      temp.terms.add(t.antiDifferentiate()); 
    }
    
    // the last term is the integration constant
    if (integrationConstant.equals(RatNum.ZERO)) {
      // omit adding the term
    } else {
      temp.terms.add(new RatTerm(integrationConstant, 0));
    }
  ...
  public double integrate(double lowerBound, double upperBound) {
    ...
    for (RatTerm t : anti.terms) {
      result += (t.eval(upperBound) - t.eval(lowerBound));
    }
  ...
  public RatPoly copy() {
    RatPoly temp = new RatPoly(); // need a deep copy
    for (RatTerm t : this.terms) {
      temp.terms.add(new RatTerm(t.getCoeff(), t.getExpt()));
    }
    return temp;
  }
}
