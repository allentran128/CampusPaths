...
public class Box implements Iterable<Ball> {
    ...
    /**
     * maxVolume is the max volume the box can hold
     */
    ...
    public boolean add(Ball b) {
        if (!this.ballContainer.contains(b) && 
                ((b.getVolume() + this.ballContainer.getVolume()) <= this.maxVolume)) {
            this.ballContainer.add(b);
            return true;
        } else {
            /**
             * Either:
             * ballContainer holds Ball b already or,
             * adding in Ball b will exceed the maxVolume 
             */
            return false;
        }
    }
    ...
    public Iterator<Ball> getBallsFromSmallest() {
        ...
        Collections.sort(lst, new Comparator<Ball>() {
            @Override
